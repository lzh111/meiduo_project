...
class Brand(BaseModel):
    ...
    logo = models.ImageField(default='',verbose_name='Logo图片')
...
class SPU(BaseModel):
    """商品SPU"""
    name = models.CharField(max_length=50, verbose_name='名称')
    brand_id = models.ForeignKey(Brand, on_delete=models.PROTECT, verbose_name='品牌')
    category1_id = models.ForeignKey(GoodsCategory, on_delete=models.PROTECT, related_name='cat1_spu', verbose_name='一级类别')
    category2_id = models.ForeignKey(GoodsCategory, on_delete=models.PROTECT, related_name='cat2_spu', verbose_name='二级类别')
    category3_id = models.ForeignKey(GoodsCategory, on_delete=models.PROTECT, related_name='cat3_spu', verbose_name='三级类别')
...
# sku.spu: 和sku对象关联的spu对象
# sku.spu_id: 和sku对象关联的spu对象的id
# sku.category: 和sku对象关联的三级分类的对象
# sku.category_id: 和sku对象关联的三级分类对象的id
# sku.specs.all(): 获取和sku对象关联的sku规格的数据
class SKU(BaseModel):
    """商品SKU"""
    name = models.CharField(max_length=50, verbose_name='名称')
...
class SPUSpecification(BaseModel):
    """商品SPU规格"""
    spu_id = models.ForeignKey(SPU, on_delete=models.CASCADE, related_name='specs', verbose_name='商品SPU')
...
class SpecificationOption(BaseModel):
    """规格选项"""
    spec_id = models.ForeignKey(SPUSpecification, related_name='options', on_delete=models.CASCADE, verbose_name='规格')
